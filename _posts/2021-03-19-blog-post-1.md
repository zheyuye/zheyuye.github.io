...
$$
\mathcal{D}=\left\{{x}_{i}, y_{i}\right\}_{i=1 \ldots N}
$$
...
$$
\mathcal{D}^{\prime}=\left\{{x}_{i}^{\prime}, {y}_{i}^{\prime}\right\}_{i=1 \ldots N}
$$

基于原有集合$\mathcal{D}$和增强集合$\mathcal{D}^{\prime}$，去学习参数$p_{\theta}(\cdot)$:

$$
\theta^{*}=\underset{\theta}{\arg \min } \sum_{\left({x}_{i}, y_{i}\right) \in \mathcal{D}} \mathcal{L}\left(p_{\theta}\left({x}_{i}\right), y_{i}\right)+\sum_{\left({x}_{i}^{\prime}, y_{i}^{\prime}\right) \in \mathcal{D}^{\prime}} \mathcal{L}\left(p_{\theta}\left({x}_{i}^{\prime}\right), y_{i}^{\prime}\right)
$$
...
$$
{x}_{i}^{\prime}=\text { BackTrans }\left({x}_{i}\right)
$$
...
\mathcal{R}_{\mathrm{CS}}\left(p_{\theta}\left({x}_{i}\right), p_{\theta}\left({x}_{i}^{\prime}\right)\right)
$$的方式进行正则化, 保证对于$(x, x^{\prime})$, $p_{\theta}{{\theta}{x{i}$有一致的概率输出. Loss Fucntion通常采用KL散度.
...
$$
\begin{array}{c}
\mathcal{R}_{\mathrm{AT}}\left({x}_{i}, \tilde{{x}}_{i}, y_{i}\right)=\mathcal{L}\left(p_{\theta}\left(\tilde{{x}}_{i}\right), y_{i}\right), \text { s.t. }\left\|\tilde{{x}}_{i}-{x}_{i}\right\| \leq \epsilon \\
...
$$
\hat{{x}}_{i} \approx {x}_{i}+\epsilon \frac{{g}}{\|{g}\|_{2}}, \text { where } {g}=\nabla_{{x}_{i}} \mathcal{L}\left(p_{\theta}\left({x}_{i}\right), y_{i}\right)
$$
...
其中 $M=\left(p_{\theta}\left({x}_{i}\right)+p_{\theta}\left(\hat{{x}}_{i}\right)\right) / 2$ 
...
\mathcal{R}_{\mathrm{CT}}\left({q}_{i}, {k}_{i}, \mathcal{M}\right) &=-\log \frac{\exp \left(\operatorname{sim}\left({q}_{i}, {k}_{i}\right) / \tau\right)}{\sum_{{k}_{j} \in \mathcal{M} \cup\left\{{k}_{i}\right\}} \exp \left(\operatorname{sim}\left({q}_{i}, {k}_{j}\right) / \tau\right)}
\end{aligned}
$$

其中$\gamma$是temperature, $\mathcal{M}$是memory bank, $\operatorname{sim}(\cdot)$选取Cosine similarity.  第一项  $\mathcal{R}_{\mathrm{CT} }\left({q}_{i}, {k}_{i}, \mathcal{M}\right) $ 基于原始样本计算而来, 表示self-contrastive loss; 第二项 $ \mathcal{R}_{\mathrm{CT}}\left({q}_{i}^{\prime}, {k}_{i}, \mathcal{M}\right) $ 由增强样本 $ x_i^{\prime} $  得到, 表示augment-contrastive loss. 其表达的想法显而易见–**原始样本和增强样本对相对于memory bank中的负样本，在学习表征特征时应保持更近的距离.**  这个loss为我们提供了global information, 使得整个训练得到了宏观正则化.
