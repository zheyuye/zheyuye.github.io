...
$$的方式进行正则化, 保证对于$(x, x^{\prime})$, $p_{\theta}\boldsymbol{x}_{i}$, $p_{\theta}\boldsymbol{x^{\prime}}_{i}$有一致的概率输出. Loss Fucntion通常采用KL散度.
